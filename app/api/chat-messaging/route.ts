 ...
 */
const getGlobalMemoryServer = async (
  userId: string
): Promise<{ memories: string[]; enabled: boolean } | null> => {
  try {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isGuest) {
      // Force guest users to use Gemini 2.5 Flash Lite regardless of what model is sent
    ...
    if (isGuest) {
      ...
      if (experimental_attachments && experimental_attachments.length > 0) {
        return new Response(
          JSON.stringify({
            error:
              "File attachments are not available for guest users. Please sign up to use this feature.",
      ...
      if (
        ...
        return new Response(
          JSON.stringify({
            error:
              "Conversation styles are not available for guest users. Please sign up to use this feature.",
    ...
    if (userId && !isGuest) {
      try {
        ...
        console.error(
          "Failed to get custom profile, project prompt, or global memory:",
          error
        );
      }
    }
    ...
    console.log(
      `📂 Collected ${conversationFiles.length} files from conversation history:`,
      conversationFiles.map((f) => ({
        fileName: f.fileName,
        fileType: f.fileType,
        contentLength: f.textContent.length,
      }))
    );
    ...
    const processedMessages = messages.map(
      (message: Record<string, unknown>) => {
        if (
          ...
          console.log(
            "Processing message with experimental_attachments:",
    ...
    if (
      ...
      globalMemory.memories.length > 0
    ) {
      systemPrompt += `\n\n--- MEMORY ---`;
    ...
    if (conversationFiles.length > 0) {
      console.log(
        `📁 Adding ${conversationFiles.length} files to conversation context`
      );
