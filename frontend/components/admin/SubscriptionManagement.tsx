...
const SubscriptionManagement: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'cancelled':
        return <Badge className="bg-yellow-100 text-yellow-800"><AlertTriangle className="w-3 h-3 mr-1" />Cancelled</Badge>;
      case 'expired':
        return <Badge className="bg-red-100 text-red-800"><XCircle className="w-3 h-3 mr-1" />Expired</Badge>;
      case 'on_hold':
        return <Badge className="bg-orange-100 text-orange-800"><AlertTriangle className="w-3 h-3 mr-1" />On Hold</Badge>;
      case 'failed':
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage all premium subscriptions and user preferences</p>
        </div>
          ...
          <Card key={index} className="p-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="font-semibold">{item.user?.email || 'Unknown User'}</h3>
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-mono text-xs break-all">{item.subscription?.subscriptionId || 'N/A'}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Customer ID</p>
                <p className="font-mono text-xs break-all">{item.subscription?.customerId || 'N/A'}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Amount</p>
                <p>{item.subscription?.currency || 'N/A'} {item.subscription?.amount || 'N/A'}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Period End</p>
                <p>{item.subscription?.currentPeriodEnd ? new Date(item.subscription.currentPeriodEnd).toLocaleDateString() : 'N/A'}</p>
              </div>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <p className="font-mono text-xs break-all">{item.subscription?.lastPaymentId || 'N/A'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p>{item.subscription?.createdAt ? new Date(item.subscription.createdAt).toLocaleDateString() : 'N/A'}</p>
              </div>
            </div>
              ...
              <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-semibold">{item.preferences.freeCredits || 0}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Premium Models</p>
                    <p className="font-semibold">{item.preferences.premiumCredits || 0}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Super Premium</p>
                    <p className="font-semibold">{item.preferences.superPremiumCredits || 0}</p>
                  </div>
                </div>
              </div>
            )}
      ...
      {subscriptions.length === 0 && (
        <Card className="p-8 text-center">
          ...
          <p className="text-muted-foreground">No premium subscriptions are currently active.</p>
        </Card>
      )}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={editForm.tier} onValueChange={(value: string) => setEditForm({...editForm, tier: value})}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  <Label htmlFor="free-credits" className="text-sm">Free Models</Label>
                  <Input
                    id="free-credits"
                    type="number"
                    ...
                    onChange={(e) => setEditForm({
                      ...editForm,
                      credits: { ...editForm.credits, free: parseInt(e.target.value) || 0 }
                    })}
                  />
                </div>
                <div>
                  <Label htmlFor="premium-credits" className="text-sm">Premium Models</Label>
                  <Input
                    id="premium-credits"
                    type="number"
                    ...
                    onChange={(e) => setEditForm({
                      ...editForm,
                      credits: { ...editForm.credits, premium: parseInt(e.target.value) || 0 }
                    })}
                  />
                </div>
                <div>
                  <Label htmlFor="super-premium-credits" className="text-sm">Super Premium Models</Label>
                  <Input
                    ...
                    onChange={(e) => setEditForm({
                      ...editForm,
                      credits: { ...editForm.credits, superPremium: parseInt(e.target.value) || 0 }
                    })}
                  />
                </div>
              </div>
