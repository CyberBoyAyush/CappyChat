...
import { ChevronDown, ChevronRight, Folder, FolderOpen, MoreHorizontal, Plus, Palette } from 'lucide-react';
import { cn } from '@/lib/utils';
import { ProjectData } from '@/frontend/hooks/useProjectManager';
import { ThreadData, ThreadOperations } from '@/frontend/components/panel/ThreadManager';
...
interface ProjectFolderProps {
  ...
  onProjectUpdate: (projectId: string, name: string, description?: string) => Promise<void>;
  onProjectDelete: (projectId: string, reassignThreadsToProjectId?: string) => Promise<void>;
  onProjectColorChange: (projectId: string, colorIndex: number) => Promise<void>;
...
export const ProjectFolder: React.FC<ProjectFolderProps> = ({
  ...
  const colorIndex = (project as any).colorIndex ?? (project.id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % PROJECT_COLORS.length);
  ...
  const handleRenameConfirm = async (newName: string, newDescription?: string) => {
    try {
  ...
  return (
    <div className="mb-2">
      ...
      <div
        className={cn(
          'flex items-center justify-between px-2 py-1.5 rounded-lg cursor-pointer group/project',
      ...
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {isExpanded ? (
            <FolderOpen className={cn("h-4 w-4 flex-shrink-0", projectColor.icon)} />
          ) : (
            <Folder className={cn("h-4 w-4 flex-shrink-0", projectColor.icon)} />
          )}
      ...
      {isExpanded && threads.length > 0 && (
        <div className="ml-6 mt-1 space-y-1">
          {threads
            ...
            .map((thread, index) => (
            <SidebarMenuItem key={`project-${project.id}-${thread.id}-${index}`}>
              <ThreadListItem
                threadData={thread}
            ...
            <DialogDescription>
              ...
              {threads.length > 0 && (
                <span className="block mt-2">
                  This project contains {threads.length} conversation{threads.length > 1 ? 's' : ''}.
                  They will be moved to the main conversation list.
                </span>
              )}
            </DialogDescription>
          </DialogHeader>
